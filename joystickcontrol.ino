...
void joystick_c::update() {
	...
	unsigned long I2Ctimeout=millis()+250; // set 250ms timeout
	...
	if(Wire.available() == 6) { // ready to go
		...
		if(bytestoingore>0) {
			...
			filteredX=nunchuck_joyx-120; // todo recal this at startup 22-124-228
			 filteredY=nunchuck_joyy-120; // todo recal this at startup 227-131-41
        #ifdef debugOut_joystick
	        Serial.print("x");
                Serial.println (filteredX); //rbw ts
                delay(200);
                Serial.print("y");
                Serial.println (filteredY);//rbw ts
                delay(200);
        #endif
	
        //filter dead zone
			if(filteredX > -20 && filteredX < 20) { //rbw orig 20 20
				filteredX=0;
			}
			if(filteredY > -20 && filteredY < 20) { //rbw orig 20 20
				filteredY=0;
			}
        // filter >100
			if(filteredX > 100) {
			...
			// update virtual DPAD where analog direction is converted into up down left right events
			updateDPAD();
